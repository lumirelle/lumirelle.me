import { resolve } from 'node:path'
import UnoCSS from 'unocss/vite'
import { defineConfig } from 'vite'
import Inspect from 'vite-plugin-inspect'
import Exclude from 'vite-plugin-optimize-exclude'
import { AutoImports } from './plugins/auto-imports.ts'
import { Await } from './plugins/await.ts'
import { Components } from './plugins/components.ts'
import { Icon } from './plugins/icon.ts'
import { Markdown } from './plugins/markdown.ts'
import { Svg } from './plugins/svg.ts'
import { VueRouter } from './plugins/vue-router.ts'
import { Vue } from './plugins/vue.ts'

const promises: Promise<any>[] = []

export default defineConfig({
  resolve: {
    alias: [{ find: '~/', replacement: `${resolve(import.meta.dirname, 'src')}/` }],
  },
  optimizeDeps: {
    include: ['vue', 'vue-router', '@vueuse/core', 'dayjs', 'dayjs/plugin/localizedFormat'],
  },
  plugins: [
    UnoCSS(),
    VueRouter(),
    Vue(),
    Markdown(promises),
    AutoImports(),
    Components(),
    Inspect(),
    Icon(),
    Svg(),
    Exclude(),
    Await(promises),
  ],

  build: {
    rolldownOptions: {
      onwarn(warning, next) {
        if (warning.code !== 'UNUSED_EXTERNAL_IMPORT') {
          next(warning)
        }
      },
    },
  },

  ssgOptions: {
    formatting: 'minify',
  },
})
